...
public class Utils {
	...
	public int[] coords(Entity curEnt) {
		return new int[] { (int) Math.floor(curEnt.posX), (int) Math.floor(curEnt.posY) - 2,
				(int) Math.floor(curEnt.posZ) };
	}
	...
	public NBTTagCompound jsonToNbt(String json) {
		NBTBase base = null;
		try {
			base = JsonToNBT.func_150315_a(json);
	...
	public ItemStack item(ItemStack toItem, NBTTagCompound tag) {
		if (toItem != null && tag != null) {
			toItem.setTagCompound(
					collideNbt(toItem.hasTagCompound() ? toItem.stackTagCompound : new NBTTagCompound(), tag));
		}
		return toItem;
	}
	...
	public Stream<Entity> nearEntityes(int radius) {
		return world().loadedEntityList.stream()
				.filter(e -> player().getDistanceToEntity((Entity) e) <= radius && e != player());
	}
	...
	public List<String> tabList() {
		return ((List<GuiPlayerInfo>) player().sendQueue.playerInfoList).stream().map(r -> r.name)
	...
	public List<TileEntity> nearTiles() {
		...
		if (chunkProvider instanceof ChunkProviderClient) {
			for (Chunk chunk : (List<Chunk>) ReflectionHelper.getPrivateValue(ChunkProviderClient.class,
					(ChunkProviderClient) chunkProvider, 3)) {
	...
	public boolean isDoubleChest(TileEntity tile) {
		if (tile instanceof TileEntityChest) {
			...
			int y = pos[1];
			int z = pos[2];
			if (tile(x - 1, y, z) instanceof TileEntityChest || tile(x + 1, y, z) instanceof TileEntityChest
					|| tile(x, y, z - 1) instanceof TileEntityChest || tile(x, y, z + 1) instanceof TileEntityChest) {
				return true;
			}
		}
		return false;
	...
	public boolean isNodeAura(TileEntity e) {
		try {
			return Class.forName("thaumcraft.common.tiles.TileNode").isInstance(e);
		} catch (Exception ex) {
			return false;
		}
	}
