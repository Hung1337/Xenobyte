...
public class Config {
	...
	public static void load() {
		try {
			...
			XenoLogger.getLogger().error("Config сброшен изза ошибки чтения из файла: " + e.getMessage());
			save();
		}
	}
				...
				for (Field field : clazz.getDeclaredFields()) {
					if (field.isAnnotationPresent(Cfg.class)) {
						...
						try {
							...
							XenoLogger.getLogger().error("ошибка при сохранении поля ModuleConfig: " + module + " -> "
			...
			if (module != null) {
				FLDS.forEach((FLD, DATA) -> {
					...
					try {
						...
						XenoLogger.getLogger().error("ошибка при чтении поля ModuleConfig: " + module + " -> " + cField
								+ " [" + e.getMessage() + "]");
						e.printStackTrace();
					}
				});
