...
public class Ceiling extends CheatModule {
	...
	private boolean isAcceptableMaterial(Material material) {
		return material == Material.air || material == Material.water || material == Material.web
				|| material == Material.lava;
	}

	private boolean isAcceptableBlocks(int x, int y, int z) {
		return isAcceptableMaterial(utils.block(x, y, z).getMaterial())
				&& isAcceptableMaterial(utils.block(x, y + 1, z).getMaterial());
	}
	...
	public void onPerform(PerformSource src) {
		...
		do
			y += step;
		while (isAcceptableBlocks(x, y, z) && isAcceptableCoordinates(x, y, z));
		do
			y += step;
		while (!isAcceptableBlocks(x, y, z) && isAcceptableCoordinates(x, y, z));
		do
