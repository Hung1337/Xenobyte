	...
	public void onPerform(PerformSource src) {
		...
		try {
			if (Class.forName("betterquesting.blocks.TileSubmitStation").isInstance(checkTile)) {
				...
				Class.forName("betterquesting.network.PacketSender")
						.getMethod("sendToServer", Class.forName("betterquesting.api.network.QuestingPacket"))
						.invoke(Class.forName("betterquesting.network.PacketSender").getField("INSTANCE").get(null),
								Class.forName("betterquesting.api.network.QuestingPacket")
										.getConstructor(ResourceLocation.class, NBTTagCompound.class)
										.newInstance(new ResourceLocation("betterquesting:edit_station"), root));
			}
		} catch (Exception e) {
		}
	}
