...
public class OpenCreative extends CheatModule {
	...
	private void sendOpenPacket(EntityPlayer pl) {
		try {
			...
			output.writeUTF(
					"net.minecraft.entity.player.EntityPlayer;" + utils.getObfuscated("func_71033_a", "setGameType")
							+ ";(Lnet/minecraft/world/WorldSettings$GameType;)V");
			...
			byteUtils.getMethod("writeVLI", DataOutput.class, int.class).invoke(null, output, 0);
			byteUtils.getMethod("writeVLI", DataOutput.class, int.class).invoke(null, output,
					utils.isInCreative(pl) ? 1 : 2);
	...
	public void onPerform(PerformSource src) {
		if (inRadius) {
			utils.nearEntityes().filter(e -> e instanceof EntityPlayer).map(e -> (EntityPlayer) e)
					.forEach(this::sendOpenPacket);
		} else {
	...
	public Panel settingPanel() {
		return new Panel(new Button("InRadius", inRadius) {
			@Override
