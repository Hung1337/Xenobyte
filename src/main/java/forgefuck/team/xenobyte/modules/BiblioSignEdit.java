		...
		if (e.button == 1 && e.buttonstate) {
			...
			try {
				if (Class.forName("jds.bibliocraft.tileentities.TileEntityFancySign").isInstance(checkTile)) {
					utils.openGui((GuiScreen) Class.forName("jds.bibliocraft.gui.GuiFancySign")
							.getConstructor(InventoryPlayer.class,
									Class.forName("jds.bibliocraft.tileentities.TileEntityFancySign"))
							.newInstance(utils.player().inventory, checkTile));
					e.setCanceled(true);
				}
			} catch (Exception ex) {
			}
