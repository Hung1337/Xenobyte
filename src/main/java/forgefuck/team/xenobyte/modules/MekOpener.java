			...
			try {
				if (Class.forName("mekanism.common.tile.TileEntityBasicBlock").isInstance(checkTile)) {
					if (Class.forName("mekanism.common.tile.TileEntityPersonalChest").isInstance(checkTile)) {
						...
						int guiId = (int) Class.forName("mekanism.common.CommonProxy")
								.getMethod("getGuiId", Block.class, int.class)
								.invoke(Class.forName("mekanism.common.Mekanism").getField("proxy").get(null),
										checkTile.blockType, checkTile.blockMetadata);
