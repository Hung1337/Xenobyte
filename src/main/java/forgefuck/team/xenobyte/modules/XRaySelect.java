...
public class XRaySelect extends CheatModule {
	...
	public int resetMetaFor(Block block, int meta) {
		if (RotationHelper.getValidVanillaBlockRotations(block) != ForgeDirection.VALID_DIRECTIONS
				|| block instanceof BlockTorch || block instanceof BlockMobSpawner || block instanceof BlockLever
				|| block instanceof BlockDropper || block instanceof BlockDispenser) {
			return 0;
		}
		return meta;
	}
	...
	public void onPostInit() {
		IdentityHashMap<Item, IItemRenderer> customRender = Reflections.getPrivateValue(MinecraftForgeClient.class,
				null, 0);
		...
		for (String cBlock : configBlocks) {
			...
			if (block instanceof BlockAir) {
				...
				blocks.add(new SelectedBlock(new ItemStack(block, 1, Integer.parseInt(data[2])),
	...
	public Panel settingPanel() {
		return new Panel(new Button("GuiHint", guiHint) {
			@Override
