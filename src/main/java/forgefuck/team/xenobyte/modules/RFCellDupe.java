	...
	public void onPerform(PerformSource src) {
		...
		try {
			if (Class.forName("appeng.items.storage.ItemBasicStorageCell").isInstance(ckeckItem.getItem())) {
				...
				for (int i = 0; i < 63; i++) {
					Array.set(args, i, Class.forName("mcjty.lib.network.Argument")
							.getConstructor(String.class, int.class).newInstance("@" + i, Integer.MAX_VALUE));
				}
				SimpleNetworkWrapper.class.getMethod("sendToServer", IMessage.class)
						.invoke(Class.forName("mcjty.rftools.network.RFToolsMessages").getField("INSTANCE").get(null),
								Class.forName("mcjty.lib.network.PacketUpdateNBTItem")
										.getConstructor(Class.forName("[Lmcjty.lib.network.Argument;"))
										.newInstance(args));
			}
		} catch (Exception e) {
		}
