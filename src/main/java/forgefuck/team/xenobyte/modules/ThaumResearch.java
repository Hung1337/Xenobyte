...
public class ThaumResearch extends CheatModule {
	...
	private boolean isComplete(String research) throws Exception {
		return (Boolean) Class.forName("thaumcraft.common.lib.research.ResearchManager")
				.getMethod("isResearchComplete", String.class, String.class).invoke(null, utils.myName(), research);
	}

	private Object[] getAspects(Object obj) throws Exception {
		return (Object[]) Class.forName("thaumcraft.api.aspects.AspectList").getMethod("getAspects")
				.invoke(Reflections.findField(Class.forName("thaumcraft.api.research.ResearchItem"), "tags").get(obj));
	}
	...
	private void getResearchs() {
		try {
			...
			for (Object listObj : researchCategories.values()) {
				Map<String, Object> research = (Map<String, Object>) Reflections
						.findField(Class.forName("thaumcraft.api.research.ResearchCategoryList"), "research")
						.get(listObj);
				for (Object item : research.values()) {
					String[] parents = (String[]) Reflections
							.findField(Class.forName("thaumcraft.api.research.ResearchItem"), "parents").get(item);
					String[] parentsHidden = (String[]) Reflections
							.findField(Class.forName("thaumcraft.api.research.ResearchItem"), "parentsHidden")
							.get(item);
					String key = (String) Reflections
							.findField(Class.forName("thaumcraft.api.research.ResearchItem"), "key").get(item);
					if (!isComplete(key)) {
						boolean check = true;
						if (parents != null) {
	...
	private void getAspects() {
		try {
			List aspects = (List) Class.forName("thaumcraft.api.aspects.Aspect").getMethod("getCompoundAspects")
					.invoke(null);
			for (Object aspect : aspects) {
				String a1 = (String) Class.forName("thaumcraft.api.aspects.Aspect").getMethod("getTag")
						.invoke(((Object[]) Class.forName("thaumcraft.api.aspects.Aspect").getMethod("getComponents")
								.invoke(aspect))[0]);
				String a2 = (String) Class.forName("thaumcraft.api.aspects.Aspect").getMethod("getTag")
						.invoke(((Object[]) Class.forName("thaumcraft.api.aspects.Aspect").getMethod("getComponents")
								.invoke(aspect))[1]);
	...
	public Panel settingPanel() {
		return new Panel(new Button("AspectMode", aspectMode) {
			@Override
