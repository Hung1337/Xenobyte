	...
	private boolean shutDownPacket(TileEntity comp) {
		try {
			if (Class.forName("li.cil.oc.common.tileentity.Case").isInstance(comp)) {
				Class.forName("li.cil.oc.client.PacketSender$")
						.getMethod("sendComputerPower", Class.forName("li.cil.oc.common.tileentity.traits.Computer"),
								boolean.class)
						.invoke(Class.forName("li.cil.oc.client.PacketSender$").getField("MODULE$").get(null), comp,
								state);
				return true;
			} else if (Class.forName("li.cil.oc.common.tileentity.Rack").isInstance(comp)) {
				for (int rack = 0; rack <= 3; rack++) {
					Class.forName("li.cil.oc.client.PacketSender$")
							.getMethod("sendServerPower", Class.forName("li.cil.oc.common.tileentity.Rack"), int.class,
									boolean.class)
							.invoke(Class.forName("li.cil.oc.client.PacketSender$").getField("MODULE$").get(null), comp,
									rack, state);
				}
