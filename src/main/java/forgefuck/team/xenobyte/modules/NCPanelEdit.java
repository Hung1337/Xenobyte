...
public class NCPanelEdit extends CheatModule {
	...
	public void mouseEvent(MouseEvent e) {
		if (e.button == 1 && e.buttonstate) {
			...
			try {
				Map blocks = (Map) Class.forName("shedar.mods.ic2.nuclearcontrol.blocks.BlockNuclearControlMain")
						.getDeclaredField("subblocks").get(null);
				for (Object block : blocks.values()) {
					if (block.getClass().getMethod("getTileEntity").invoke(block).getClass().isInstance(checkTile)) {
						utils.openGui((GuiScreen) block.getClass()
								.getMethod("getClientGuiElement", TileEntity.class, EntityPlayer.class)
								.invoke(block, checkTile, utils.player()));
						e.setCanceled(true);
						break;
					}
				}
