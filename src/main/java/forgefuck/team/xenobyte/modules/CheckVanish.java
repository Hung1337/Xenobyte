...
public class CheckVanish extends CheatModule {
		...
		simpleHandler = new INetHandlerStatusClient() {
			...
			public void handleServerInfo(S00PacketServerInfo infoPacket) {
				...
				if (response.func_151318_b() != null) {
					...
					outMessage = new WidgetMessage(CheckVanish.this, count + (withNames ? " " + vanishNames : ""),
							count > 0 ? WidgetMode.FAIL : WidgetMode.SUCCESS);
				}
			}
	...
	private void simpleRequest(String ip) {
		try {
			...
			NetworkManager manager = NetworkManager.provideLanClient(InetAddress.getByName(addr.getIP()),
					addr.getPort());
	...
	public Panel settingPanel() {
		return new Panel(new Button("WithNames", withNames) {
			@Override
			...
			public String elementDesc() {
				return "По возможности выводить имена (beta)";
			}
		});
	}
