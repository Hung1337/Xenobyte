...
public class ModuleHandler {
	...
	public ModuleHandler() {
		...
		new Config(this);
		enabledList = workingModules().filter(m -> m.cfgState || m.getMode() == PerformMode.ON_START)
	...
	public void bind(CheatModule module, Button button, int key) {
		...
		if (conflicted != null && !conflicted.equals(module)) {
			widgets().widgetMessage(new WidgetMessage(
					"Кейбинд [" + Keyboard.getKeyName(key) + "] конфликтует с "
							+ (conflicted.hasCategory() ? conflicted.getCategory() + "/" : "") + conflicted,
					WidgetMode.FAIL));
		} else {
