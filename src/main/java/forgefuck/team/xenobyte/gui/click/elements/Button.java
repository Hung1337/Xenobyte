...
public class Button extends TextElement {
	...
	public Button(String text, Object value) {
		this(text, value, ElementAligment.CENTER, Colors.NONE, Colors.SKY, Colors.SKY, Colors.SKY, Colors.WHITE,
				Colors.WHITE);
	}

	public Button(String text, Object value, ElementAligment aligment, int bgColor, int hoverColor, int selectColor,
			int textColor, int selectTextColor, int hoverTextColor) {
	...
	@Override
	public void onDraw() {
		render.GUI.drawRect(getX(), getY(), getMaxX(), getMaxY(),
				isSelected() ? selectColor : isHovered() ? hoverColor : bgColor);
		String out = value != null && isHovered() ? value : getText();
		render.GUI.xenoFont().drawString(out, getTextX(out), getTextY(),
				isSelected() ? selectTextColor : isHovered() ? hoverTextColor : textColor);
	}
