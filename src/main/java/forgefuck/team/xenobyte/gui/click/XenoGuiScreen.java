...
public class XenoGuiScreen extends GuiScreen {
	...
	public XenoGuiScreen(ModuleHandler HAND) {
		...
		mainPanel = new Panel(new Button(Xeno.mod_name, HAND.xenoGui().getKeyName(), ElementAligment.CENTER,
				Colors.ORANGE, Colors.ORANGE, Colors.ORANGE, Colors.WHITE, Colors.WHITE, Colors.WHITE) {
			@Override
		...
		for (Category CAT : Category.values()) {
			List<CheatModule> categoryed = HAND.categoryedModules().filter(m -> m.getCategory() == CAT)
					...
					Button modButton = new Button(MOD.getName(), MOD.getKeyName()) {
						...
						public void onDraw() {
							...
							if (settingsPanel != null) {
								int startX = isShowing(settingsPanel) ? getMaxX() + settingsPanel.getWidth()
										: getMaxX() - 1;
								render.GUI.drawRect(startX, getY(), startX + 1,
										isShowing(settingsPanel) ? settingsPanel.getMaxY() : getMaxY(), Colors.ORANGE);
							}
						}
						...
						public void onHovered() {
							...
							if (settingsPanel != null && !isShowing(settingsPanel)) {
								...
								settingsPanel.setPos(
										getMaxX() + settingsPanel.getWidth() > GuiScaler.scaledScreenWidth()
												? getX() - settingsPanel.getWidth()
												: getMaxX(),
										getY() + settingsPanel.getHeight() > GuiScaler.scaledScreenHeight()
												? getMaxY() - settingsPanel.getHeight()
												: getY());
